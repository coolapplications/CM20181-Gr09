...
public class BlocksFragment extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View v = inflater.inflate(R.layout.fragment_blocks, container, false);

        String[] blocks = new String[29];
        for(int i = 0; i < blocks.length; i++){

            blocks[i] = "Bloque " + String.valueOf(i+1);

        }

        Spinner spinner = (Spinner) v.findViewById(R.id.spinner_blocks);
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this.getActivity(), android.R.layout.simple_spinner_item, blocks);
        adapter.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);
        spinner.setAdapter(adapter);

        btn = (Button) v.findViewById(R.id.btnBloques);
